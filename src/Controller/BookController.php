...
class BookController extends AbstractController
{
    ...
    public function search(
        ...
        if ($query !== '' && mb_strlen($query) >= 3) {
            ...
            // Filtrer par note minimale si besoin
            if ($noteMin !== null) {
                $livres = array_filter($livres, function (Book $book) use ($noteMin) {
                    $avg = $book->getAverageRating();
                    return $avg !== null && $avg >= (float) $noteMin;
                });
                $livres = array_values($livres);
            }

            // Récupérer les auteurs des livres trouvés
            $auteurs = array_filter(array_map(fn($book) => $book->getAuthor(), $livres));
            $auteurs = array_unique($auteurs, SORT_REGULAR);
            $auteurs = array_slice($auteurs, 0, 10); // max 10 auteurs
            ...
            $utilisateurs = array_filter($allUsers, function($user) use ($search, $currentUser) {
                if ($currentUser && $user->getId() === $currentUser->getId()) return false;
                $pseudo = mb_strtolower($user->getPseudo());
                return str_contains($pseudo, $search) || levenshtein($pseudo, $search) <= 2;
            });

            $utilisateurs = array_values($utilisateurs);
            $utilisateurs = array_slice($utilisateurs, 0, 10); // max 10 utilisateurs

            /* === PAGINATION LIVRES === */
            $totalPages = ceil(count($livres) / $limit);
            $livres = array_slice($livres, ($page - 1) * $limit, $limit);
        }
